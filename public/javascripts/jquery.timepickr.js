...
$.widget('ui.timepickr', {
	...
	getValue: function(type) {
			return $('.'+ type +'.ui-state-hover', this.menu).add($('.'+type+':first', this.menu)).eq(0).text();
	},
	...
	_redraw: function() {
		...
		ols.each(function(i) {
			...
			var pos = prevOL.find('.ui-state-hover:visible').add(prevOL.find('li:visible:first')).position();
