class SignupController < ApplicationController
  ...
  def do_signup
    ...
    Company.transaction do
      ...
      @plan_sub = PlanSubscription.new(:user => @user, :company => @company, :plan => @plan)
      @plan_sub.next_bill_date = Time.now + (@plan.days_to_first_bill).days if @plan.days_to_first_bill
      ...
      raise ActiveRecord::Rollback if !@company.valid? or !@user.valid? or !@account.valid? or !@plan_sub.valid? or @terms != 1
