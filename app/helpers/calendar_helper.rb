module CalendarHelper
  ...
  def build_calendar_prev_next_links(provider, daterange)
    ...
    next_link   = link_to("Next", calendar2_when_start_path(:provider_id => provider.id, :provider_type => provider.tableize, :when => next_when, :start_date => next_start), :class => 'admin')
    prev_link   = link_to("Previous", calendar2_when_start_path(:provider_id => provider.id, :provider_type => provider.tableize, :when => next_when, :start_date => prev_start), :class => 'admin')

    Hash[:prev_link => prev_link, :next_link => next_link]
  end

  def build_calendar_times(hours, minute_interval=15)
    # build minute increments
    minute_increments = (0..59).step(60/(60/minute_interval)).collect { |i| i }
    
    hours.inject([]) do |array, hour|
      minute_increments.each do |minute|
        array.push("#{sprintf("%d", hour)}:#{sprintf("%02d", minute)}")
      end
      array
    end
  end
