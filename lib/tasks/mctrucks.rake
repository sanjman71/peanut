namespace :mctrucks do
  ...
  task :users_resources do
    ...
    if (@owner = User.find_by_email("mrtrucks@peanut.com") )
      ...
      @owner = User.create(:name => "Mr Trucks", :email => "mrtrucks@peanut.com", :password => "peanut", :password_confirmation => "peanut")
      @owner.register!
      @owner.activate!
    end
        
    if @mctrucks && (@moving_van = @mctrucks.user_providers.find_by_name("Moving Van"))
  ...
  task :company do
    ...
    if @mctrucks.blank?  
      ...
      @mctrucks = Company.create(:name => "McTrucks", :time_zone => "Central Time (US & Canada)")
    end

    if @mctrucks.subscription.blank?
      # create subscription in 'active' state
      @subscription = @mctrucks.create_subscription(:user => @owner, :plan => @max_plan)
      @subscription.active!
    end
    ...
    @mctrucks.user_providers.push(@owner) unless @mctrucks.user_providers.include?(@owner)
    @mctrucks.resource_providers.push(@moving_van)
  end
  ...
  task :services do
    ...
    @rental.resource_providers.push(@moving_van) unless @rental.user_providers.include?(@moving_van)
